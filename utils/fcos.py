...
class BoxCoder(object):
    ...
    @staticmethod
    def encode(grids,targets):
        '''
        :param grids:  [grids_num, 2]
        :param targets: [target_num,4]
        :return: [grid_num,target_num,4]
        '''
        left=grids[:,None,0]-targets[None,:,0]
        top=grids[:,None,1]-targets[None,:,1]
        right=targets[None,:,2]-grids[:,None,0]
        bottom=targets[None,:,3]-grids[:,None,1]
        reg_targets = torch.stack([left, top, right, bottom], dim=2)
        return reg_targets

    @staticmethod
    def decoder(grids,predicts):
        '''
        :param grids:  [grid_num, 2]
        :param predicts: [bs,grid_num,4]
        :return: [bs,grid_num,4]
        '''
        predicts_lt=grids-predicts[...,:2]
        predicts_rb=grids+predicts[...,2:]
        output = torch.cat([predicts_lt, predicts_rb], dim=-1)
        return output
