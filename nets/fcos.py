...
class BasicBlocks(nn.Module):
    def __init__(self,ch_in,ch_out,num_blocks,dcn_head=False):
        super(BasicBlocks, self).__init__()
        assert "dcn_head is not support now, coming soon"
        bones=list()
        for i in range(num_blocks):
            if i==0:
                conv_func=CGR(ch_in,ch_out,3,1)
            elif i == num_blocks - 1 and dcn_head:
                conv_func = CGR(ch_out, ch_out, 3, 1)
            else:
                conv_func = CGR(ch_out, ch_out, 3, 1)
...
class FCOSHead(nn.Module):
    def __init__(self,in_channel=256,
        ...
        self.scales=nn.ModuleList([Scale(init_val=1.) for _ in range(self.layer_num)])
        self.grids=[torch.zeros(size=(0,4))]*self.layer_num
        ...
        self.gaussian_mu=torch.zeros(size=(self.num_cls,2))
        self.gaussian_theta=torch.ones(size=(self.num_cls,2))
        self.gaussian=nn.Parameter(torch.cat([self.gaussian_mu,self.gaussian_theta],dim=1),requires_grad=True)
    ...
    def forward(self,xs):
        ...
        if self.training:
            return cls_outputs, reg_outputs, center_outputs, self.grids, self.gaussian
        else:
            bs=xs[0].shape[0]
            output=list()
...
if __name__ == '__main__':
    ...
    net(input_tensor)
    cls_outputs, reg_outputs, center_outputs, grids, gaussian = net(input_tensor)
    for grid in center_outputs:
        print(grid.shape)
